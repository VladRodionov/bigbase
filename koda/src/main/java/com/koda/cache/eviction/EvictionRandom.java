...
/*******************************************************************************
 * Copyright (c) 2011 Vladimir Rodionov. All Rights Reserved
 ...
 * OF THE SOFTWARE, EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE
 * IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, OR
 * NON-INFRINGEMENT. CARRIERIQ SHALL NOT BE LIABLE FOR ANY DAMAGES SUFFERED
 * BY LICENSEE AS A RESULT OF USING, MODIFYING OR DISTRIBUTING THIS SOFTWARE OR
 * ITS DERIVATIVES.
 ...
 * The Class LRUPolicy.
 */
public class EvictionRandom extends EvictionAlgo {

	/** The Constant LOG. */
	@SuppressWarnings("unused")
	private final static Logger LOG = Logger.getLogger(EvictionRandom.class);
	/**
	 * Instantiates a new LRU policy.
	 *
	 * @param parent the parent
	 */
	public EvictionRandom(OffHeapCache parent) {
		super(parent);
	...
	public long doEviction(EvictionData data) throws NativeMemoryException {
		...
		while(evictedPtr == 0L && toEvict < data.getSize()){
	
			evictedPtr = evict(data, toEvict++, Reason.ALGO);
			if(evictedPtr != 0){
				data.reset(toEvict-1);
			}
		}
