 ...
 */
public class TimedSpinLockControl<T> implements LockControl<T>{
    ...
    private long spinTimeout; // in nanoseconds
    
    public TimedSpinLockControl(AtomicIntegerFieldUpdater<T> updater, long spinTimeout){
    ...
    public boolean lock(T obj)
    {
        ...
        while(!updater.compareAndSet(obj, UNLOCKED, LOCKED) ){
            ...
            if(System.nanoTime() - startTime < spinTimeout){
                int counter =0;
                while(counter++ < count);
                count+= backoffInt;
            } else{
    ...
    public boolean tryLock(T obj, long timeOut /*nanoseconds*/) {
        long count = backoffMin;
        long timeout = backoffMinSleep;
        long spinTimeout = this.spinTimeout;
        if(timeOut < spinTimeout) spinTimeout = timeOut; 
        long startTime = System.nanoTime();
